...
public class TipoContactoController : ApiBaseController
{
    ...
    {
        var contactTypes = await _unitOfWork.TipoContactos.GetByIdAsync(id);
    ...
    public async Task<ActionResult<TipoContacto>> Put(int id, [FromBody]TipoContacto tipoContacto){
        if(tipoContacto == null)
            return NotFound();
        _unitOfWork.TipoContacto.Update(tipoContacto);
        await _unitOfWork.SaveAsync();
        return tipoContacto;
    }
    // [DELETE]
    ...
    public async Task<IActionResult> Delete(int id){
        ...
        _unitOfWork.TipoContacto.Remove(tipoContacto);
        await _unitOfWork.SaveAsync();
        return NoContent();
    }
}